...
// ---- Tool: chat ----
server.tool("keyhub_chat", "Send a chat completion through KeyHub's multi-provider AI gateway. Supports OpenAI, Anthropic, Google, and Mistral models. Use provider/model format (e.g. openai/gpt-4o, anthropic/claude-3-5-sonnet-20241022).", {
    model: z
        .string()
        .describe('Model ID in provider/model format, e.g. "openai/gpt-4o" or "anthropic/claude-3-5-sonnet-20241022"'),
    messages: z
        .array(z.object({
...
// ---- Tool: list_models ----
server.tool("keyhub_list_models", "List all AI models available through your KeyHub instance, grouped by provider.", {}, async () => {
    try {
        ...
        if (models.length === 0) {
            return { content: [{ type: "text", text: "No models available. Add provider keys in KeyHub first." }] };
        }
        const grouped = {};
...
// ---- Tool: get_dashboard ----
server.tool("keyhub_dashboard", "Get KeyHub dashboard metrics: total spend, request count, average latency, active keys, cost forecasting, and provider breakdown.", {
    days: z
        .number()
        .min(1)
        .max(90)
...
// ---- Tool: get_logs ----
server.tool("keyhub_logs", "Get recent API request logs from KeyHub with filtering. Shows model, status, tokens, cost, and latency per request.", {
    limit: z
        .number()
        .min(1)
        .max(100)
    ...
    try {
        ...
        if (logs.length === 0) {
            return { content: [{ type: "text", text: "No logs found matching your criteria." }] };
        }
...
// ---- Tool: get_usage ----
server.tool("keyhub_usage", "Get usage analytics from KeyHub — spending trends, top models, and request patterns over time.", {
    days: z
        .number()
        .min(1)
        .max(90)
...
async function main() {
    if (!KEYHUB_API_KEY && !KEYHUB_SESSION_TOKEN) {
        console.error("Warning: Neither KEYHUB_API_KEY nor KEYHUB_SESSION_TOKEN is set.\n" +
